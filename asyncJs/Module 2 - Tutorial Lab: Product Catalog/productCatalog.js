...
function updateTable(tableId, productArray) {
    ...
    tableBody.addEventListener('click', (e) => {
        if(e.target && e.target.id === 'examine') {
            e.stopPropagation();
            processSearch(parseInt(e.target.parentNode.firstChild.innerHTML));
        }
    });
    ...
    for(let i = 0; i < productArray.length; i++) {
        ...
        th4.appendChild(document.createTextNode("Examine"));
        th4.setAttribute("id","examine"); // to event delegation
        tr.appendChild(th1);
        tr.appendChild(th2);
        tr.appendChild(th3);
        tr.appendChild(th4);
...
function updateExaminedText (product) {
    let outputString = `Product Id: ${product.id} <br>
...
function handleSearch () {
    processSearch(parseInt(document.getElementById('input').value));
}
